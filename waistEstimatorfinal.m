function waistEstimatorfinal
    ...
    while flag == 0
     if (depth.logical())
            ...
            indices = findPointsInROI(ptcl_out,[-0.25 0.25 0.1 0.8 -0.1 0.15]); %0.20
    ...
    for i = 1:10000
        ...
        % Divide in Depth and Color Fram
        depth = fs.get_depth_frame(); %0.005
        color = fs.get_color_frame(); %0.0005
...
%             ptcl_display.Color = lab2uint8(repmat([128 128 128],ptcl_display.Count,1));
            ...
            indices_left = findPointsInROI(ptcl_zone,[mean_zone(1)-0.12 mean_zone(1)-0.07 range mean_zone(3)+0.02 mean_zone(3)+0.07]);
            ...
            indices_right = findPointsInROI(ptcl_zone,[mean_zone(1)+0.07 mean_zone(1)+0.12 range mean_zone(3)+0.02 mean_zone(3)+0.07]);
...
%             view(player,pccat([ptcl_display ptcl_zone_mean ptcl_line ptcl_lrtotal_mean ptcl_right ptcl_right_mean ptcl_left ptcl_left_mean]));
